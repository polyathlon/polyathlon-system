...
export default class DataSource {
    ...
    init() {
        if (this.items.length) {
            let itemId = sessionStorage.getItem('currentSportsDisciplinesAgeGroup')
            let item
            if (itemId) {
                item = this.items.find((item) => item._id == itemId)
            }
            item ??= this.items[0]
            sessionStorage.setItem('currentSportsDisciplinesAgeGroup', item._id)
    ...
    getCurrentItem(){
        const item = sessionStorage.getItem('currentSportsDisciplinesAgeGroup')
        if (item) {
            return this.items.find(p => p._id === item)
        }
        else {
            sessionStorage.setItem('currentSportsDisciplinesAgeGroup', this.items[0]?._id)
            return this.items?.[0]
        }
    }
