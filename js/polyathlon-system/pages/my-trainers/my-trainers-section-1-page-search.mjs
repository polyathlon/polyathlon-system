...
import TrainerCategoriesDataset from '../my-trainer-categories/my-trainer-categories-dataset.mjs'
import TrainerCategoriesDataSource from '../my-trainer-categories/my-trainer-categories-datasource.mjs'
...
class MyTrainersSection1PageSearch extends BaseElement {
    static get properties() {
        return {
            version: { type: String, default: '1.0.0' },
            trainerCategoriesDataSource: { type: Object, default: null },
    ...
    render() {
        return html`
            ...
            <div class="container">
                <simple-input id="lastName" label="${lang`Last name`}:" icon-name="user" @icon-click=${this.gotoPersonalPage} .dataSource=${this.findDataSource} button-name="user-magnifying-glass-solid" @button-click=${this.findTrainerByName} .listLabel=${this.trainerListLabel} .listIcon=${this.trainerListIcon} .listStatus=${this.trainerListStatus.bind(this)} .value=${this.item?.lastName} @input=${this.validateInput} @select-item=${this.sportsmanChoose}></simple-input>
                ...
                <simple-input id="sortOrder" label="${lang`Sort order`}:" icon-name="order-number-solid" @icon-click=${this.copyToClipboard} .value=${this.item?.sortOrder} @input=${this.validateInput}></simple-input>
                <simple-input id="trainerPC" label="${lang`Trainer PC`}:" .dataSource=${this.findDataSource} icon-name="trainer-pc-solid" @icon-click=${this.copyToClipboard} button-name="user-magnifying-glass-solid"  @button-click=${this.findSportsman} .value=${this.item?.trainerPC} @input=${this.validateInput} @select-item=${this.sportsmanChoose} ></simple-input>
                ...
                <simple-select id="category" label="${lang`Category`}:" icon-name="trainer-category-solid" @icon-click=${() => this.showPage('my-trainer-categories')} .dataSource=${this.trainerCategoriesDataSource} .value=${this.item?.category} @input=${this.validateInput}></simple-select>
            </div>
        `;
    }
    ...
    gotoPersonalPage() {
        if (!this.item?.trainerId) {
            return
        }
        location.hash = "#my-trainer";
        location.search = `?trainer=${this.item?.trainerId.split(':')[1]}`
    }
    ...
    async findSportsman(e) {
        ...
        if (!value) {
            ...
            sportsman = await TrainerDataset.getItem(value)
        } else if (target.value.includes("-")) {
            sportsman = await TrainerDataset.getItemBySportsmanPC(value)
            if (sportsman.rows.length === 0) {
                this.showDialog('Такой спортсмен не найден')
                return
            }
            ...
            sportsman = await TrainerDataset.getItemByLastName(value)
            if (sportsman.rows.length >= 0) {
        ...
        if (sportsman) {
            ...
            this.item.trainerId = sportsman._id
    ...
    async firstUpdated() {
        super.firstUpdated();
        this.trainerCategoriesDataSource = new TrainerCategoriesDataSource(this, await TrainerCategoriesDataset.getDataSet())
...
customElements.define("my-trainers-section-1-page-search", MyTrainersSection1PageSearch);