...
export default class DataSet {
    ...
    static #fetchGetItems(token) {
        return fetch(`https://${HOST}:4500/api/regions`)
    }
    ...
    static fetchAddItem(token, item) {
        return fetch(`https://${HOST}:4500/api/region`, {
            method: "POST",
            headers: {
    ...
    static #fetchGetItem(token, itemId) {
        return fetch(`https://${HOST}:4500/api/region/${itemId}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://${HOST}:4500/api/region/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://${HOST}:4500/api/region/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
