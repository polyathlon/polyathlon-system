...
export default class DataSet {
    ...
    static #fetchGetItems() {
        return fetch(`https://${HOST}:4500/api/federation-member-positions`)
    }
    ...
    static fetchAddItem(token, item) {
        return fetch(`https://${HOST}:4500/api/federation-member-position`, {
            method: "POST",
            headers: {
    ...
    static #fetchGetItem(itemId) {
        return fetch(`https://${HOST}:4500/api/federation-member-position/${itemId}`)
    }
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://${HOST}:4500/api/federation-member-position/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://${HOST}:4500/api/federation-member-position/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
