...
export default class DataSource {
    ...
    getCurrentItem(){
        const item = sessionStorage.getItem('currentCityType')
        if (item) {
            return this.items.find(p => p._id === item)
        }
        else {
            sessionStorage.setItem('currentCityType', this.items[0]?._id)
            return this.items?.[0]
        }
    }

    setCurrentItem(item) {
        sessionStorage.setItem('currentCityType', item._id)
