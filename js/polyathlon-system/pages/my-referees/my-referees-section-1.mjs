...
class MyRefereesSection1 extends BaseElement {
    ...
    render() {
        return html`
            ...
            <header id="competition-header"><p>Referee ${this.currentItem?.lastName + ' ' + this.currentItem?.firstName[0] + '.' + this.currentItem?.middleName[0] + '.'}</p></header>
            <header id="property-header">${this.#pageName}</header>
            <div class="left-layout">
                ${this.dataSource?.items?.map((item, index) =>
                    html `<project-button
                                label=${item.lastName + ' ' + item.firstName[0] + '.' + item.middleName[0] + '.'}
                                title=${item._id}
