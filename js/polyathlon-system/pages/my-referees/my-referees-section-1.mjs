...
import './my-referees-section-1-page-search.mjs'
import './my-referees-section-1-page-filter.mjs'
...
class MyRefereesSection1 extends BaseElement {
    ...
    constructor() {
        super();
        this.$partid = 'MyRefereesSection1'
    ...
    #page1() {
        return html`
            <my-referees-section-1-page-1 class="page" .oldValues=${this.oldValues} .item=${this.currentItem}></my-referees-section-1-page-1>
        `
    }

    #pageSearch() {
        return html`
            <my-referees-section-1-page-search class="page" .item=${this.currentSearch}></my-referees-section-1-page-search>
        `;
    }

    #pageFilter() {
        return html`
            <my-referees-section-1-page-filter class="page" .item=${this.currentFilter} .oldValues=${this.oldFilterValues}></my-referees-section-1-page-filter>
        `;
    }
    ...
    sectionName(section) {
        if (this.currentPage == 0 ) {
            section.iconName = this.currentItem?.gender == 0 ?  "referee-man-solid" : "referee-woman-solid"
            return section
    ...
    leftHeaderTitle() {
        return lang`Referees` + (this.dataSource?.items?.length ? ' ('+ this.dataSource?.items?.length +')' : '')
    }
    ...
    async saveToFile(blob, fileName) {
        if (window.navigator && window.navigator.msSaveOrOpenBlob) {
            window.navigator.msSaveOrOpenBlob(blob, 'referee-qr.svg');
        } else {
            const options = {
