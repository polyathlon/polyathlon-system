...
export default class DataSource {
    ...
    constructor(component, dataSet) {
        ...
        this.items = [...dataSet].sort((a, b) => a.type - b.type)
        this.state = States.BROWSE
    }

    async init() {
        if (this.items.length) {
            let itemId = sessionStorage.getItem('currentProfileDocument')
            let item
            if (itemId) {
                item = this.items.find((item) => item.id == itemId)
            }
            item ??= this.items[0]
            sessionStorage.setItem('currentProfileDocument', item.id)
    ...
    async setCurrentItem(item) {
        sessionStorage.setItem('currentProfileDocument', item._id)
        this.component.currentItem = item
    }
    ...
    deleteFrom(item) {
        ...
        if (this.items.length === 1) {
            sessionStorage.removeItem('currentProfileDocument')
            this.component.currentItem = {}
        }
