...
export default class DataSource {
    ...
    async init() {
        if (this.items.length) {
            let itemId = sessionStorage.getItem('currentProfileSportsmanRequest')
            let item
            if (itemId) {
                item = this.items.find((item) => item.id == itemId)
            }
            item ??= this.items[0]
            sessionStorage.setItem('currentProfileSportsmanRequest', item.id)
    ...
    async setCurrentItem(item) {
        sessionStorage.setItem('currentProfileSportsmanRequest', item._id)
        this.component.currentItem = item
    }
    ...
    deleteFrom(item) {
        ...
        if (this.items.length === 1) {
            sessionStorage.removeItem('currentProfileSportsmanRequest')
            this.component.currentItem = {}
        }
