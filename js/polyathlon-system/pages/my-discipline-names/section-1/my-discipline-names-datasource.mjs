...
export default class DataSource {
    ...
    async init() {
        if (this.items.length) {
            let itemId = sessionStorage.getItem('currentDisciplineName')
            let item
            if (itemId) {
                item = this.items.find((item) => item.id == itemId)
            }
            item ??= this.items[0]
            sessionStorage.setItem('currentDisciplineName', item.id)
    ...
    deleteFrom(item) {
        ...
        if (this.items.length === 1) {
            sessionStorage.removeItem('currentDisciplineName')
            this.component.currentItem = {}
        }
