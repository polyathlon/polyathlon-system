...
export default class DataSet {
    ...
    static #fetchGetItems(token) {
        return fetch('https://localhost:4500/api/referee-positions', {
            headers: {
    ...
    static fetchAddItem(token, item) {
        return fetch(`https://localhost:4500/api/referee-position`, {
            method: "POST",
            headers: {
                'Authorization': `Bearer ${token}`,
    ...
    static #fetchGetItem(token, itemId) {
        return fetch(`https://localhost:4500/api/referee-position/${itemId}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://localhost:4500/api/referee-position/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://localhost:4500/api/referee-position/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
