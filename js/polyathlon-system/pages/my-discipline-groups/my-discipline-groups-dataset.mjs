...
export default class DataSet {
    ...
    static #fetchGetItems(token) {
        return fetch('https://localhost:4500/api/discipline-groups', {
            headers: {
    ...
    static fetchAddItem(token, item) {
        return fetch(`https://localhost:4500/api/discipline-group`, {
            method: "POST",
            headers: {
                'Authorization': `Bearer ${token}`,
    ...
    static #fetchGetItem(token, itemId) {
        return fetch(`https://localhost:4500/api/discipline-group/${itemId}`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        })
    ...
    static #fetchSaveItem(token, item) {
        return fetch(`https://localhost:4500/api/discipline-group/${item._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    static #fetchDeleteItem(token, item) {
        return fetch(`https://localhost:4500/api/discipline-group/${item._id}?rev=${item._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
