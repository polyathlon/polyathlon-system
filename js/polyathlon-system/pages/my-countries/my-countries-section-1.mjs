...
class MyCountriesSection1 extends BaseElement {
    ...
    fetchGetItems(token) {
        return fetch('https://localhost:4500/api/countries', {
            headers: {
    ...
    getCurrentItem(){
        ...
        if (item) {
            ...
            sessionStorage.setItem('currentCountry', this.dataSet[0]._id)
    ...
    fetchSaveItem(token) {
        return fetch(`https://localhost:4500/api/country/${this.currentItem._id}`, {
            method: "PUT",
            headers: {
              'Authorization': `Bearer ${token}`,
              'Content-Type': 'application/json;charset=utf-8'
    ...
    fetchDeleteItem(token) {
        return fetch(`https://localhost:4500/api/country/${this.currentItem._id}?rev=${this.currentItem._rev}`, {
            method: "DELETE",
            headers: {
                'Authorization': `Bearer ${token}`
            }
