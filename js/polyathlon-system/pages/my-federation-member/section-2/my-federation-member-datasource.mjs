...
export default class DataSource {
    ...
    async init() {
        if (this.items.length) {
            let itemId = sessionStorage.getItem('currentFederationMemberRequest')
            let item
            if (itemId) {
                item = this.items.find((item) => item.id == itemId)
            }
            item ??= this.items[0]
            sessionStorage.setItem('currentFederationMemberRequest', item.id)
    ...
    async setCurrentItem(item) {
        sessionStorage.setItem('currentFederationMemberRequest', item._id)
        this.component.currentItem = item
        this.component.currentPage = item.type - 1
    }
    ...
    async saveItem(item) {
        this.component.currentItem = await DataSet.saveItem(item)
        const index = this.items.indexOf(item)
        this.items[index] = this.component.currentItem
        this.state = States.BROWSE
    }
    ...
    deleteFrom(item) {
        ...
        if (this.items.length === 1) {
            sessionStorage.removeItem('currentFederationMemberRequest')
            this.component.currentItem = {}
        }
