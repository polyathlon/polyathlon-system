...
class MyCompetitionSection2Table1 extends BaseElement {
    ...
    // { name: "regionTeamNumber", label: lang`Region team`, },
    // { name: "clubTeamNumber", label: lang`Club team`, },

    constructor() {
        super()
        this.columns = [[
            { name: "sportsman", label: lang`Sportsman`, textAlign: 'left'},
            { name: "category", label: lang`Short sports category`, },
            { name: "birthday", label: lang`Birthday`, },
            { name: "club", label: lang`Sports club`, },
            { name: "teamMember", label: lang`Region team member`, },
            { name: "clubMember", label: lang`Club team member`, },
            { name: "sportsNumber", label: lang`Sportsman number`, },
        ]]
    }
        ...
        if (changedProps.has('sportsmenDataSource')) {
            this.items = this.sportsmenDataSource.items.map(item => {
                return {
                    ...
                    regionTeamNumber: item.regionTeamNumber,
                    clubTeamNumber: item.clubTeamNumber,
                    sportsNumber: item.sportsNumber,
                }
            }).sort((a, b) => this.regionCompare(a, b) || this.clubCompare(a, b) || this.genderCompare(a,b) )
            // sort((a, b) => (this.parent?.name?.championship ? a.gender - b.gender : a.gender - b.gender || a.ageGroupOrder - b.ageGroupOrder) || b.points - a.points || this.getResult(b) - this.getResult(a))
        }
    }
    ...
    async firstUpdated() {
        ...
        if (this.parent?.name?.championship) {
            this.groups = [
                ...
                { name: "region", label: item => item.region, title: lang`Region` },
            ]
        } else {
            this.groups = [
...
customElements.define("my-competition-section-2-table-1", MyCompetitionSection2Table1);