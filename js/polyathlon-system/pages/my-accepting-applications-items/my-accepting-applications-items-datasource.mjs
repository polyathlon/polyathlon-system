import DataSet from "./my-accepting-applications-items-dataset.mjs";
...
export default class DataSource {
    ...
    getCurrentItem(){
        const item = sessionStorage.getItem('currentRegistration')
        if (item) {
            return this.items.find(p => p._id === item)
        }
        else {
            sessionStorage.setItem('currentRegistration', this.items[0]?._id)
            return this.items?.[0]
        }
    }

    setCurrentItem(item) {
        sessionStorage.setItem('currentRegistration', item._id)
