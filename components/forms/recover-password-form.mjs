...
class RecoverPasswordForm extends BaseElement {
                ...
                <div class="form-header">
                    <div class="form-tabs no-select">
                        <div class="form-tab" selected>
                            <span id="db-tab" class="form-tab-link select">Recover Password</span>
                        </div>
                    </div>
    ...
    async loginValidation(e) {
        ...
        try {
            await RecoverPasswordForm.checkUsername(user)
            this.isLoginError = false
    ...
    async emailValidation(e) {
        ...
        try {
            await RecoverPasswordForm.checkEmail(user)
            this.isEmailError = false
    ...
    static async checkUsername(user) {
        const response = await RecoverPasswordForm.fetchCheckUsername(user)
    ...
    static async checkEmail(user) {
        const response = await RecoverPasswordForm.fetchCheckEmail(user)
    ...
    async sendSimpleUser() {
        ...
        let response = await RecoverPasswordForm.fetchSendSimpleUser(user)
    ...
    async getSimpleUserInfo() {
        ...
        let response = await RecoverPasswordForm.fetchSimpleUserInfo(token)

        if (response.status === 419) {
            const token = await refreshToken()
            response = await RecoverPasswordForm.fetchSimpleUserInfo(token)
        }
...
customElements.define("recover-password-form", RecoverPasswordForm);